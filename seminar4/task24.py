...
# которое может собрать за один заход собирающий модуль, находясь перед некоторым кустом заданной во
# входном файле грядки.

import random

N = int(input("Введите количество кустов: "))
list = list(random.randint(0, 9) for i in range(N))
print(list)
i = 0
S = 0
result = []
for i in range(N):
    if i == N - 1:
        S = list[i] + list[i - 1] + list[0]
    else:
        S = list[i] + list[i - 1] + list[i + 1]
        result.append(S)
result.sort()
print(result)
print(f"Максимальное количество ягод за один круг собирающего устройства: {result[-1]}")
