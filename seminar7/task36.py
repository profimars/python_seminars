...
def print_operation_table(operation, num_rows=6 - 0, num_columns=6 - 0):
    matrix = [[operation(i, j) for i in range(1, num_rows + 1)] for j in range(1, num_columns + 1)]
    for element in matrix:
           print(*[f"{x:>5}" for x in element])  # 5 - размер паддинга
